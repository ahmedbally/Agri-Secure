                        ...
                        @if(@Auth::user()->permissionsGroup->add_status)
                            @foreach($GeneralWebmasterSections as $headerWebmasterSection)
                                @if(in_array($headerWebmasterSection->id,$data_sections_arr))
                                    ...
                                    ?>
                                    @if($headerWebmasterSection->id!=14)
                                        <a class="dropdown-item"
                                           href="{{route("topicsCreate",$headerWebmasterSection->id)}}">
                                           <span><i class="material-icons">{!! $LiIcon !!}</i> &nbsp;{!! str_replace("backLang.","",trans('backLang.'.$headerWebmasterSection->name)) !!}</span>
                                       </a>
                                    @else
                                        <a class="dropdown-item"
                                           href="{{route("sectionsCreate",$headerWebmasterSection->id)}}">
                                           <span><i class="material-icons">{!! $LiIcon !!}</i> &nbsp;{!! str_replace("backLang.","",trans('backLang.'.$headerWebmasterSection->name)) !!}</span>
                                       </a>
                                    @endif
                                @endif
