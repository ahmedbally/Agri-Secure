                        ...
                        @foreach($Sections as $Section)
                            ...
                            @foreach($Section->fatherSections as $subSection)
                                ...
                                @if($WebmasterSection->id == 14)
                                    @foreach($subSection->fatherSections as $thirdSec)
                                        <?php
                                        if ($thirdSec->$title_var != "") {
                                            $title = $thirdSec->$title_var;
                                        } else {
                                            $title = $thirdSec->$title_var2;
                                        }
                                        ?>
                                        <tr>
                                            <td><label class="ui-check m-a-0">
                                                    <input type="checkbox" name="ids[]" value="{{ $thirdSec->id }}"><i
                                                            class="dark-white"></i>
                                                    {!! Form::hidden('row_ids[]',$thirdSec->id, array('class' => 'form-control row_no')) !!}
                                                </label>
                                            </td>
                                            <td>
                                                @if($thirdSec->photo !="")
                                                    <div class="pull-right">
                                                        <img src="{{ URL::to('uploads/sections/'.$thirdSec->photo) }}"
                                                             style="height: 30px" alt="{{ $title }}">
                                                    </div>
                                                @endif
                                                <img src="{{ URL::to('backEnd/assets/images/treepart_'.trans('backLang.direction').'.png') }}"
                                                     class="submenu_tree">
                                                <!-- {!! Form::text('row_no_'.$thirdSec->id,$thirdSec->row_no, array('class' => 'form-control row_no','id'=>'row_no')) !!} -->
                                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; + &nbsp;
                                                @if($thirdSec->icon !="")
                                                    <i class="fa {!! $thirdSec->icon !!} "></i>
                                                @endif
                                                {!! $title   !!}</td>
                                            <td class="text-center">
                                                {!! $thirdSec->visits  !!}
                                            ...
                                            <td class="text-center">
                                                <i class="fa {{ ($thirdSec->status==1) ? "fa-check text-success":"fa-times text-danger" }} inline"></i>
                                            </td>
                                            <td class="text-center">
                                                @if(@Auth::user()->permissionsGroup->edit_status)
                                                    <a class="btn btn-sm success"
                                                       href="{{ route("sectionsEdit",["webmasterId"=>$WebmasterSection->id,"id"=>$thirdSec->id]) }}">
                                                        <small><i class="material-icons">
                                                                &#xe3c9;</i> {{ trans('backLang.edit') }}
                                                        </small>
                                                    </a>
                                                ...
                                                @if(@Auth::user()->permissionsGroup->delete_status)
                                                    <button class="btn btn-sm warning" data-toggle="modal"
                                                            data-target="#m-{{ $thirdSec->id }}" ui-toggle-class="bounce"
                                                            ui-target="#animate">
                                                        <small><i class="material-icons">
                                                                &#xe872;</i> {{ trans('backLang.delete') }}
                                                        </small>
                                        ...
                                        <!-- .modal -->
                                        <div id="m-{{ $thirdSec->id }}" class="modal fade" data-backdrop="true">
                                            <div class="modal-dialog" id="animate">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">{{ trans('backLang.confirmation') }}</h5>
                                                    ...
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn dark-white p-x-md"
                                                                data-dismiss="modal">{{ trans('backLang.no') }}</button>
                                                        <a href="{{ route("sectionsDestroy",["webmasterId"=>$WebmasterSection->id,"id"=>$thirdSec->id]) }}"
                                                           class="btn danger p-x-md">{{ trans('backLang.yes') }}</a>
                                                    </div>
                                                </div><!-- /.modal-content -->
                                            </div>
